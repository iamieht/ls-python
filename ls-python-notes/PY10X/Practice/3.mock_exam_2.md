...
The concept demonstrated here is Truthiness, and how values are evaluated in a Boolean context. In this code snippet, `0` evaluates as `False` or is `falsy`, so the else branch is executed. In Python the `falsy` values are: all numbers which values are zero. Empty collections, empty strings, empty sequences and the Boolean `False`.
